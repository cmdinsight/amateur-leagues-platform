        ...
        <section className="rounded-xl border border-slate-200 p-5">
          ...
          <div className="mb-3 flex flex-wrap gap-2">
            {tournament.groups.map((g) => (
              <span key={g.id} className="rounded-full bg-slate-100 px-3 py-1 text-sm text-slate-700">
                {g.name}
              </span>
            ))}
              ...
              return (
                <form key={m.id} action={submit} className="rounded-xl border border-slate-200 p-4">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2 text-sm">
                    <div>
                      <p className="mb-1 text-xs font-semibold uppercase text-slate-400">Goles — {m.homeTeam.name}</p>
                      {m.homeTeam.rosterSpots.map((rs) => (
                        <div key={rs.id} className="flex items-center justify-between py-0.5">
                          ...
                          <input type="number" min={0} defaultValue={0} name={`homeGoals_${rs.playerId}`} className="input w-14 text-center" />
                        </div>
                      ))}
                    </div>
                    <div>
                      <p className="mb-1 text-xs font-semibold uppercase text-slate-400">Goles — {m.awayTeam.name}</p>
                      {m.awayTeam.rosterSpots.map((rs) => (
                        <div key={rs.id} className="flex items-center justify-between py-0.5">
                          <span>{rs.player.fullName}</span>
                          <input type="number" min={0} defaultValue={0} name={`awayGoals_${rs.playerId}`} className="input w-14 text-center" />
                        </div>
                      ))}
                    </div>
                  </div>
